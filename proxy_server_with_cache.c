...
int handle_request(int clientSocket, ParsedRequest *request, char *tempReq)
{
	...
	if (temp_buffer == NULL) {
		printf("Memory allocation failed for temp_buffer\n");
		free(buf);
	...
	while (bytes_send > 0)
	{
		...
		// FIXED: Bounds checking for temp_buffer_index
...
void *thread_fn(void *socketNew)
{
	...
	if (tempReq == NULL) {
		printf("Memory allocation failed for tempReq\n");
	...
	// FIXED ISSUE 1: Adding null terminator for tempReq
	...
	if (temp != NULL)
	{
		...
		while (pos < size)
		{
			...
			int bytes_to_copy = (size - pos) > MAX_BYTES ? MAX_BYTES : (size - pos);
			for (int i = 0; i < bytes_to_copy; i++)
			{
				response[i] = temp->data[pos];
				pos++;
			}
			// FIXED ISSUE 2: Send only actual bytes needed, not always MAX_BYTES
			send(socket, response, bytes_to_copy, 0);
		}
		printf("Data retrived from the Cache\n\n");
		printf("%s\n\n", response);
		// FIXED ISSUE 3: Proper cleanup and exit
		shutdown(socket, SHUT_RDWR);
		close(socket);
		free(buffer);
		free(tempReq);
...
int main(int argc, char *argv[])
{
	...
	// FIXED ISSUE 4: Thread array index wrapping issue
	...
	while (1)
	{
		...
		i++;
		// FIXED ISSUE 4: Wrap i around when it reaches MAX_CLIENTS to prevent overflow
		if (i >= MAX_CLIENTS) {
			i = 0;
		}
	}
...
void remove_cache_element()
{
	...
	cache_element *p = NULL;	 // Cache_element Pointer (Prev. Pointer)
	cache_element *q;	 // Cache_element Pointer (Next Pointer)
	cache_element *temp; // Cache element to remove
	cache_element *temp_prev = NULL; // Previous pointer for temp
	...
	if (head != NULL)
	{ // Cache != empty
		// FIXED ISSUE 5: Corrected LRU algorithm - properly find oldest element
		temp = head;
		q = head;
		
		// Find the element with minimum lru_time_track
		while (q != NULL) {
		...
		// Find the previous element of temp
		if (temp != head) {
			p = head;
			while (p->next != temp) {
				p = p->next;
...
int add_cache_element(char *data, int size, char *url)
{
	...
	else
	{
		...
		if (element == NULL) {
			printf("Memory allocation failed for cache element\n");
			temp_lock_val = pthread_mutex_unlock(&lock);
			return 0;
		}
		...
		if (element->data == NULL) {
			printf("Memory allocation failed for cache element data\n");
			free(element);
			temp_lock_val = pthread_mutex_unlock(&lock);
			return 0;
		}
		...
		if (element->url == NULL) {
			printf("Memory allocation failed for cache element url\n");
			free(element->data);
			free(element);
			temp_lock_val = pthread_mutex_unlock(&lock);
			return 0;
